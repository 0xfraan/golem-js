        ...
        this.server = net.createServer((client) => {
            ...
            const flushWebSocketBuffer = () => {
                ...
                if (state.wsBuffer.length > 0) {
                    ws$1.send(buffer.Buffer.concat(state.wsBuffer), {
                        binary: true,
                        mask: true,
                    });
                }
            ...
            const ws$1 = new ws.WebSocket(this.wsUrl, { headers: { authorization: `Bearer ${this.appKey}` } });
            // OPEN HANDLERS
            ws$1.on("open", () => {
            ...
            client.on("data", async (chunk) => {
                ...
                if (!state.wsReady) {
                    ...
                    ws$1.send(chunk, { binary: true, mask: true });
                }
            });
            // Send data to the client or buffer if it's not ready yet
            ws$1.on("message", (message) => {
...
exports.WebSocketBrowserStorageProvider = WebSocketBrowserStorageProvider;
...
//# sourceMappingURL=shared-ChS9baDB.js.map
